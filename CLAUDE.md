...
| `handzoo-ui` — the review surface | **Working.** `handzoo-ui out/` serves a local page: page image and emitted text side by side, page list with gate state. **Two save actions** — *Fix transcription* writes `edited`, *Edit my notes* writes `authored`. Bound to 127.0.0.1; page images never leave the machine. |
...
| Tests | **253**, plus the frozen `baseline/` corpus as a regression suite. CI never calls a model. |
